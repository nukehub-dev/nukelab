...
Docker image definitions for the NukeLab platform runtime environments (`base`,
`workspace`, and `dev`). Domain-specific scientific computing images
(`radiation-transport`, `gpu`, `moose`, `cardinal`, `openfoam`) live in the
separate `nukelab-environments` repository.
...
- Images are built via `scripts/environments/build-base.sh`, `scripts/environments/build-workspace.sh`, and `scripts/environments/build-dev.sh` or the CI/CD pipeline.
...
- `scripts/build-all.sh` builds the platform images (sidecar first, then base, workspace, and dev). Domain-specific environment images are built from `nukelab-environments`.

## Related repositories

- `nukelab-environments` — domain-specific scientific computing images
  (`radiation-transport`, `gpu`, `moose`, `cardinal`, `openfoam`) that extend
  the published `nukelab-workspace` and `nukelab-nuclear-base` images.
