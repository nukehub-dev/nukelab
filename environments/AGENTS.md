...
- `conda-base/` is the shared conda/Python/build foundation used by `workspace/` and by scientific toolchain images in `nukelab-environments`. Keeping it separate from `workspace/` means IDE updates do not force rebuilds of heavy C++/Fortran scientific stacks.
