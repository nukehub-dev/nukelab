...
| 2026-08-21 | Prod still builds from source; pull-based prod deploys deferred | `compose.yml` has no `image:` references to the CI-built GHCR images, so prod deploys (`git pull` + `nukelabctl up/update`) rebuild from source on the host and the published registry images have no consumer. Making prod pull tagged images (`image:` + `${NUKELAB_VERSION}` pins, rollback = re-pin) is the planned upgrade, deferred until release cadence justifies it | Deferred |
