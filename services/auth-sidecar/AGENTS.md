...
- `/auth` and `/validate` record allowed requests to an in-memory `last_activity` timestamp, exposed via `GET /activity`; the backend idle-shutdown task polls it (`app/tasks.py:_fetch_sidecar_activity`). Expired-but-authentic tokens (valid signature + audience) also record activity but are still rejected with 401. `/health`, `/metrics`, and forged or wrong-audience tokens must never update it.
