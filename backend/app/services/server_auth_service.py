...
class ServerAuthService:
    ...
    async def revoke_token(
        ...
        result = await db.execute(
            select(ServerAccessToken).where(
                and_(ServerAccessToken.jti == jti, ServerAccessToken.revoked_at.is_(None))
            )
        )
    ...
    async def is_token_revoked(self, db: AsyncSession, jti: str) -> bool:
        ...
        result = await db.execute(
            select(ServerAccessToken).where(
                and_(ServerAccessToken.jti == jti, ServerAccessToken.revoked_at.isnot(None))
            )
        )
    ...
    async def cleanup_expired_tokens(self, db: AsyncSession, max_age_days: int = 7) -> int:
        ...
        result = await db.execute(
            delete(ServerAccessToken).where(ServerAccessToken.expires_at < cutoff)
        )
        await db.commit()
    ...
    async def get_server_access_stats(
        ...
        result = await db.execute(
            select(func.count(ServerAccessToken.id)).where(
                and_(
                    ServerAccessToken.server_id == server_id, ServerAccessToken.issued_at >= day_ago
                )
            )
        )
