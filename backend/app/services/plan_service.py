...
class PlanService:
    ...
    async def list_plans(
        ...
        for plan in plans:
            ...
            role_visible = (
                user_role and plan.visible_to_roles and user_role in plan.visible_to_roles
            )
            # Direct user access
    ...
    async def update_plan(self, plan_id: str, **updates) -> ServerPlan:
        """Update server plan"""
    ...
    async def list_plan_users(self, plan_id: str) -> list[dict[str, Any]]:
        ...
        result = await self.db.execute(
            ...
            .options(
                selectinload(UserPlanAccess.user), selectinload(UserPlanAccess.granted_by_user)
            )
        )
    ...
    async def grant_user_access(
        ...
        if existing.scalar_one_or_none():
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT, detail="User already has access to this plan"
            )
        ...
        for access in accesses:
            ...
            if access.workspace:
                ...
                if access.workspace.owner:
                    item["owner_name"] = (
                        access.workspace.owner.display_name or access.workspace.owner.username
                    )
    ...
    async def grant_workspace_access(
        ...
        if existing.scalar_one_or_none():
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="Workspace already has access to this plan",
            )
