...
class UserService:
    ...
    async def update_user(
        self, user_id: str, data: Dict[str, Any], updated_by: Optional[User] = None
    ) -> User:
        """Update user"""
    ...
    async def disable_user(
        self, user_id: str, disabled: bool = True, reason: Optional[str] = None
    ) -> User:
    ...
    async def discover_users(self, search: Optional[str] = None, limit: int = 50) -> List[User]:
        ...
        """
        query = select(User).where(User.profile_visibility == "public", User.is_active == True)
