...
class MetricsCollector:
    ...
    async def _persist_metrics(self, metrics: dict):
        ...
        try:
            ...
            await db.commit()
            logger.debug(
                "Metrics collector: Saved metric to database for server %s", metrics["server_id"]
            )
    ...
    async def _broadcast_metrics(self, metrics: dict):
        ...
        try:
            ...
            await redis_client.publish(
                f"metrics:server:{metrics['server_id']}", json.dumps(metrics, default=str)
            )
            await redis_client.publish("metrics:all", json.dumps(metrics, default=str))
        except Exception:
