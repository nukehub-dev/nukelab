...
async def create_workspace(
    ...
    workspace = await service.create_workspace(
        name=request.name, description=request.description, owner_id=str(current_user.id)
    )
...
async def get_workspace(
    ...
    my_membership = next(
        (m.to_dict() for m in workspace.members if str(m.user_id) == str(current_user.id)), None
    )
    ...
    user_invitation = next(
        (
            i
            for i in workspace.invitations
            if str(i.user_id) == str(current_user.id) and i.status == "pending"
        ),
        None,
    )
...
async def update_workspace(
    ...
    await activity.log(
        ...
        details={
            "changed_fields": [
                f for f in ["name", "description", "is_active"] if getattr(request, f) is not None
            ],
            "name": request.name,
            "description": request.description,
            "is_active": request.is_active,
...
async def transfer_ownership(
    ...
    try:
        ...
        raise HTTPException(
            status_code=403, detail="You don't have permission to transfer ownership."
        )
...
async def invite_member(
    ...
    if request.role not in ("read_only", "read_write", "admin"):
        raise HTTPException(
            status_code=400, detail="Invalid role. Must be: read_only, read_write, admin"
        )
...
async def cancel_invitation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=403, detail="You don't have permission to cancel this invitation."
        )
