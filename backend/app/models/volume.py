...
class Volume(Base):
    ...
    server_mounts = relationship(
        "ServerVolume", back_populates="volume", cascade="all, delete-orphan"
    )
    workspace_associations = relationship(
        "WorkspaceVolume", back_populates="volume", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self):
        data = {
            ...
            "status": self.status,
            "server_count": len(self.server_mounts)
        ...
        if self.labels and self.labels.get("was_home_volume"):
            ...
            if "server_mounts" not in inspect(self).unloaded and self.server_mounts:
                data["is_home_volume"] = (
                    any(sm.mount_path == home_mount_path for sm in self.server_mounts)
                    if home_mount_path
                    else False
                )
            else:
