...
class TestAlertRuleModel:
    ...
    async def test_evaluate_and_to_dict(self):
        ...
        rule = AlertRule(
            name="cpu", metric_type="cpu_percent", operator=">", threshold=80.0, scope="global"
        )
...
class TestApiTokenModel:
    ...
    async def test_to_dict(self):
        ...
        token = ApiToken(user_id=uuid.uuid4(), name="test", token_prefix="pref", token_hash="hash")
        d = token.to_dict()
        assert d["name"] == "test"
...
class TestServerScheduleModel:
    ...
    async def test_to_dict(self):
        ...
        ss = ServerSchedule(server_id=uuid.uuid4(), action="start", cron_expression="0 0 * * *")
        d = ss.to_dict()
...
class TestUserModel:
    ...
    async def test_display_name_and_avatar(self):
        ...
        user = User(username="testname", email="t@example.com", first_name="John", last_name="Doe")
    ...
    async def test_avatar_url_custom(self):
        ...
        user = User(
            username="avatar", email="a@example.com", avatar_url="http://custom.com/ava.png"
        )
...
class TestWorkspaceInvitationModel:
    ...
    async def test_to_dict(self):
        ...
        wi = WorkspaceInvitation(
            workspace_id=uuid.uuid4(), invited_by=uuid.uuid4(), user_id=uuid.uuid4(), role="member"
        )
        d = wi.to_dict()
