...
class TestCacheDeletePattern:
    ...
    async def test_deletes_matching_keys(self, mock_redis):
        ...
        assert count == 2
        mock_redis.delete.assert_awaited_once_with("nukelab:cache:a:1", "nukelab:cache:a:2")
...
class TestCacheTrackKey:
    ...
    async def test_adds_key_to_set(self, mock_redis):
        ...
        mock_redis.sadd.assert_awaited_once_with("nukelab:cache:my:set", "member-key")
