...
class TestAdminVolumeList:
    ...
    @pytest.mark.asyncio
    async def test_admin_can_list_all_volumes(
    ...
    async def test_admin_filter_by_visibility(self, client: AsyncClient, admin_user, admin_token):
        ...
        resp = await client.post(
            "/api/volumes/",
            json={"name": "vis_private", "display_name": "Private Volume", "visibility": "private"},
            headers=headers,
        )
        assert resp.status_code == 201
...
class TestAdminVolumeDetail:
    ...
    async def test_admin_get_nonexistent_volume(self, client: AsyncClient, admin_user, admin_token):
        ...
        response = await client.get(
            "/api/admin/volumes/00000000-0000-0000-0000-000000000000", headers=headers
        )
        assert response.status_code == 404

    @pytest.mark.asyncio
    async def test_non_admin_cannot_get_volume_details(
...
class TestAdminVolumeUpdate:
    ...
    @pytest.mark.asyncio
    async def test_non_admin_cannot_update_volume(
        ...
        response = await client.put(
            f"/api/admin/volumes/{vol_id}", json={"display_name": "Hacked"}, headers=user_headers
        )
        assert response.status_code == 403

    @pytest.mark.asyncio
    async def test_admin_cannot_shrink_volume_below_used_size(
    ...
    @pytest.mark.asyncio
    async def test_admin_can_increase_volume_max_size(
        self, client: AsyncClient, admin_user, admin_token, db_session
    ):
...
class TestAdminVolumeDelete:
    ...
    @pytest.mark.asyncio
    async def test_non_admin_cannot_delete_volume(
        self, client: AsyncClient, test_user, user_token, admin_user, admin_token
    ):
        """Regular user should get 403."""
        admin_headers = {"Authorization": f"Bearer {admin_token}"}
...
class TestBulkVolumeActions:
    ...
    @pytest.mark.asyncio
    async def test_api_token_rejected_for_bulk_volume_action(
