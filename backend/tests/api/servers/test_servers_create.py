...
class TestCreateServerQueueing:
    ...
    @pytest.mark.asyncio
    async def test_create_server_resource_pool_queueing(self, client, user_token, test_user, db_session, test_plan_env):
...
class TestCreateServerExceptionCleanup:
    ...
    @pytest.mark.asyncio
    async def test_create_server_exception_cleanup(self, client, user_token, test_user, db_session, test_plan_env):
        ...
        assert "try again" in response.json()["detail"].lower() or "contact support" in response.json()["detail"].lower()
