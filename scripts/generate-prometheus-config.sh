...
# Generate Prometheus config and alert rules from templates.
...
NODE_EXPORTER_ROOTFS_PATH="$(resolve NODE_EXPORTER_ROOTFS_PATH /host)"
export NODE_EXPORTER_ROOTFS_PATH
...
# Single quotes are intentional: envsubst only substitutes the named variables.
envsubst '$PGBOUNCER_SCRAPE_JOBS' < "$PROJECT_ROOT/monitoring/prometheus/prometheus.yml.tpl" \
    > "$PROJECT_ROOT/monitoring/prometheus/prometheus.generated.yml"

# shellcheck disable=SC2016
# Single quotes are intentional: envsubst only substitutes the named variables.
envsubst '$NODE_EXPORTER_ROOTFS_PATH' < "$PROJECT_ROOT/monitoring/prometheus/rules/nukelab.yml.tpl" \
    > "$PROJECT_ROOT/monitoring/prometheus/rules/nukelab.generated.yml"

echo "Generated $PROJECT_ROOT/monitoring/prometheus/prometheus.generated.yml"
echo "Generated $PROJECT_ROOT/monitoring/prometheus/rules/nukelab.generated.yml"
