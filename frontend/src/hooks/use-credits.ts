...
import type { CreditSummary, CreditHistoryResponse, CreditTransaction, LowBalanceUser } from '../types/api';
...
export function useCreditActions() {
  ...
  const grantCredits = useMutation({
    mutationFn: ({ userId, amount, reason }: GrantCreditsData) =>
      api.post<{ message: string; transaction: CreditTransaction }>(`/credits/users/${userId}/grant`, { amount, reason }),
  ...
  const deductCredits = useMutation({
    mutationFn: ({ userId, amount, reason }: DeductCreditsData) =>
      api.post<{ message: string; transaction: CreditTransaction }>(`/credits/users/${userId}/deduct`, { amount, reason }),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ['credits'] });
      queryClient.invalidateQueries({ queryKey: ['users'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard'] });
