...
export function useCreateVolume() {
  ...
  return useMutation({
    mutationFn: async (data: { display_name: string; description?: string; max_size_bytes?: number }) => {
...
export function useVolumeFiles(volumeId: string | null, params: VolumeFilesParams = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await api.get<VolumeFilesResponse>(`/volumes/${volumeId}/files?${queryParams.toString()}`);
      return response;
    },
    enabled: !!volumeId,
  });
