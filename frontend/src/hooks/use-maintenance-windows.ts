...
export function useMaintenanceWindows(activeOnly?: boolean, futureOnly?: boolean) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return api.get<MaintenanceWindowListResponse>(
        `/system/maintenance-windows${query ? `?${query}` : ''}`
      )
    },
  })
}
...
export function useCreateMaintenanceWindow() {
  ...
  return useMutation({
    mutationFn: async (payload: MaintenanceWindowCreatePayload) => {
      return api.post<{ success: boolean; window: MaintenanceWindow }>(
...
export function useUpdateMaintenanceWindow() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      id: string
      payload: MaintenanceWindowUpdatePayload
    }) => {
      return api.put<{ success: boolean; window: MaintenanceWindow }>(
        `/system/maintenance-windows/${id}`,
        payload
      )
    },
    onSuccess: () => {
