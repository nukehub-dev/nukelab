...
export function useServerActions() {
  ...
  const addPendingOp = useCallback(
    (
      ...
      targetStatus: PendingOperation['targetStatus']
    ) => {
      setPendingOps((prev) => [
        ...prev.filter((op) => op.serverId !== serverId),
        { serverId, type, targetStatus },
      ])
    },
    []
  )
  ...
  useEffect(() => {
    ...
    const timers = opsWithTimeout.map(
      (op) =>
        setTimeout(
          () => {
            removePendingOp(op.serverId)
          },
          op.type === 'restart' ? 15000 : 30000
        ) // 15s for restart, 30s for start/stop
    )
  ...
  const createServer = useMutation({
    mutationFn: (data: CreateServerData) => api.post<Server>('/servers/', data),
    onSuccess: () => {
  ...
  const updateServer = useMutation({
    mutationFn: ({
      ...
      serverId: string
      data: UpdateServerData & { reason?: string }
  ...
  const startServer = useMutation({
    mutationFn: ({ serverId, reason }: { serverId: string; reason?: string }) =>
      api.post<{ message: string; server_id: string; status: Server['status'] }>(
...
export function useServerSchedules(serverId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<{ schedules: ServerSchedule[] }>(
...
export function useCreateSchedule() {
  ...
  return useMutation({
    mutationFn: ({
      ...
      serverId: string
      data: Omit<ServerSchedule, 'id' | 'server_id' | 'user_id' | 'run_count' | 'created_at'>
...
export function useServerLogs(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<{ logs: string; tail: number; status?: string }>(
        `/servers/${serverId}/logs?tail=${tail}`
      )
      return response
    },
