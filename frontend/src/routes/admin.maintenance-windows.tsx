...
function getWindowStatus(w: MaintenanceWindow): {
  ...
  if (!w.is_active)
    return { label: 'Inactive', color: 'bg-muted text-muted-foreground', icon: XCircle }
  if (w.auto_disabled)
    return { label: 'Completed', color: 'bg-emerald-500/10 text-emerald-400', icon: CheckCircle2 }
  if (w.auto_enabled)
    return { label: 'Active', color: 'bg-rose-500/10 text-rose-400', icon: AlertCircle }
  if (w.start_at <= now && w.end_at >= now)
    return { label: 'In Progress', color: 'bg-rose-500/10 text-rose-400', icon: AlertCircle }
  if (w.start_at > now) {
    if (w.notified_at)
      return { label: 'Scheduled', color: 'bg-amber-500/10 text-amber-400', icon: Bell }
...
function MaintenanceWindowsPage() {
  ...
  const minutesUntilStart = useMemo(() => {
    return startAt ? Math.max(0, Math.floor((new Date(startAt).getTime() - now) / 60000)) : 0
  }, [startAt, now])
      ...
      >
        ...
        <div className="bubble p-5 space-y-4 mb-6">
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <Wrench
                  className={cn(
                    'w-5 h-5',
                    sysConfig?.maintenance_mode ? 'text-amber-400' : 'text-emerald-400'
                  )}
                />
              </div>
              <div>
              ...
              <Switch
                ...
                onCheckedChange={(checked) => {
                  updateSysConfig.mutate({
                    ...
                    maintenance_message:
                      sysConfig?.maintenance_message || 'System under maintenance',
                  })
                }}
          ...
          <div className="flex items-start gap-2 text-xs text-muted-foreground bg-muted/20 p-3 rounded-xl">
            ...
            <span>
              Emergency maintenance overrides all scheduled windows. Admin users can still access
              the platform. Use scheduled windows for planned downtime — use this toggle only for
              unexpected issues.
            </span>
          </div>
        </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          <DialogHeader className="relative pr-8">
            <DialogTitle>
              {editing ? 'Edit Maintenance Window' : 'Schedule Maintenance'}
            </DialogTitle>
            <DialogDescription>
              {editing
                  ...
                  <>
                    ...
                    {impossibleOffsets.length > 0 && (
                      <p className="text-xs text-amber-400">
                        {impossibleOffsets.length} reminder{impossibleOffsets.length > 1 ? 's' : ''}{' '}
                        won't fire because the maintenance window starts sooner than the reminder
                        interval. They will be ignored.
                      </p>
                    )}
                    {impossibleOffsets.length === 0 && (
                      <p className="text-xs text-muted-foreground">
                        Users will receive a notification at each selected interval before
                        maintenance starts.
                      </p>
                    )}
                  </>
