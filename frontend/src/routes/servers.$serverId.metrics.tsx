  ...
  return (
    <div className="min-h-screen p-6 lg:p-10 space-y-8">
      ...
      >
        ...
        <div>
          <h1 className="text-2xl font-bold">{server.name} — Metrics</h1>
          <p className="text-sm text-muted-foreground">Resource timeline and historical metrics</p>
        </div>
      </motion.div>
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold">Server Lifecycle Timeline</h3>
          ...
          <div className="space-y-3">
            ...
            {server.started_at && (
              <div className="flex justify-between text-sm">
                ...
                <span>{formatDate(server.started_at || '')}</span>
              </div>
            )}
          </div>
          <div className="space-y-3">
            {server.allocated_cpu !== undefined && (
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Allocated CPU</span>
                <span className="font-mono">{server.allocated_cpu} cores</span>
              </div>
            )}
            {server.allocated_memory !== undefined && (
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Allocated Memory</span>
                <span className="font-mono">{server.allocated_memory} GB</span>
              </div>
            )}
            {server.external_url && (
              <div className="flex justify-between text-sm">
