...
function AdminServersContent({ enableManagement }: { enableManagement: boolean }) {
                ...
                return (
                  <div className="flex items-center gap-1">
                    ...
                    {server.status === 'stopped' && (
                      <Tooltip
                        content={isOperationPending(server.id, 'start') ? 'Starting...' : 'Start'}
                      >
                        <button
                    ...
                    {server.status === 'running' && (
                      <>
                        <Tooltip
                          content={isOperationPending(server.id, 'stop') ? 'Stopping...' : 'Stop'}
                        >
                          <button
                    ...
                    <Tooltip
                      content={isOperationPending(server.id, 'delete') ? 'Deleting...' : 'Delete'}
                    >
                      <button
                        onClick={async () => {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: servers.reduce((acc, s) => acc + (s.allocated_cpu || 0), 0),
      icon: Cpu,
  ...
  const runBulkAction = async (action: 'start' | 'stop' | 'restart' | 'delete', ids: string[]) => {
    const selectedServers = ids
      .map((id) => servers.find((s) => s.id === id))
      .filter(Boolean) as ServerType[]
