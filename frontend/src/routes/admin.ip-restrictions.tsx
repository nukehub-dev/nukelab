...
import { Shield, Trash2, AlertTriangle, Info, Lock, Unlock, Globe, X, Copy, Check, Plus, ChevronDown } from 'lucide-react';
            ...
            <div className="px-4 pb-4 space-y-3 text-sm text-muted-foreground">
              ...
              <div className="space-y-2">
                <div className="flex items-start gap-2">
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-emerald-500/10 text-emerald-400 text-xs font-bold shrink-0 mt-0.5">1</span>
                  <div>
                    <span className="text-emerald-400 font-medium">Open Mode</span>
                    <span className="block"> No restrictions. Anyone can access the platform. This is the default state.</span>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-amber-500/10 text-amber-400 text-xs font-bold shrink-0 mt-0.5">2</span>
                  <div>
                    <span className="text-amber-400 font-medium">Blocklist Mode</span>
                    <span className="block"> You add specific IPs or ranges to block. Everything else is allowed. Use this to ban known bad actors.</span>
                  </div>
                </div>
                <div className="flex items-start gap-2">
                  <span className="inline-flex items-center justify-center w-5 h-5 rounded-full bg-red-500/10 text-red-400 text-xs font-bold shrink-0 mt-0.5">3</span>
                  <div>
                    ...
                    <span className="font-semibold text-foreground">Everything else is blocked.</span>
                    <span> Use this for maximum security (e.g. restrict admin access to office VPNs only).</span>
                  </div>
                </div>
              </div>
              ...
              <div className="text-xs bg-amber-500/5 border border-amber-500/15 p-3 rounded-lg">
                ...
                {' '}Adding an allowlist entry immediately switches the platform into lockdown mode. Make sure your own IP is allowlisted first, or you will lock yourself out. Health checks, auth endpoints, and docs are always exempt from IP restrictions.
              </div>
            </div>
          </motion.div>
        )}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader className="relative pr-8">
            ...
            <DialogDescription>
              Create a new allowlist or blocklist entry.
            </DialogDescription>
            <button
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  onClick={() =>
                    setFormData({ ...formData, restriction_type: 'block' })
                  }
                ...
                <Button
                  ...
                  onClick={() =>
                    setFormData({ ...formData, restriction_type: 'allow' })
                  }
                  className="flex-1 gap-2"
                >
              ...
              {formData.restriction_type === 'allow' && (
                <div className="flex items-start gap-2 text-xs text-amber-400 bg-amber-500/10 p-3 rounded-lg border border-amber-500/20">
                  ...
                  <span>
                    <strong>Lockdown Warning:</strong> Adding an allowlist entry will block ALL traffic except allowlisted IPs. Make sure your own IP is included, or you will lock yourself out.
                  </span>
                </div>
              )}
            </div>
