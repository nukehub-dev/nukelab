...
import { useSetCreditRequestBlock } from '../../../hooks/use-credit-requests'
...
import { BulkCreditDialog } from '../bulk-credit-dialog'
import { AllowanceOverrideDialog } from '../allowance-override-dialog'
...
export function UsersTab({ focusUserId }: UsersTabProps) {
  ...
  useEffect(() => {
    if (focusUserId && usersData?.data) {
      const user = usersData.data.find((u) => u.id === focusUserId)
      if (user) queueMicrotask(() => setSelectedUser(user))
    }
  }, [focusUserId, usersData])
