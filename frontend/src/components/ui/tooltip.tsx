...
function computePosition(
  ...
  let pos = preferred;
  if (!fits[preferred]) {
    const flip: Record<TooltipPosition, TooltipPosition> = { top: 'bottom', bottom: 'top', left: 'right', right: 'left' };
    pos = fits[flip[preferred]] ? flip[preferred] : (fits.bottom ? 'bottom' : fits.top ? 'top' : 'bottom');
  }
  ...
  if (pos === 'top' || pos === 'bottom') {
    x = Math.max(VIEWPORT_PADDING + tooltipRect.width / 2, Math.min(vw - VIEWPORT_PADDING - tooltipRect.width / 2, x));
  } else {
    y = Math.max(VIEWPORT_PADDING + tooltipRect.height / 2, Math.min(vh - VIEWPORT_PADDING - tooltipRect.height / 2, y));
  }
...
export function Tooltip({
  ...
  const transform =
    actualPos === 'top' ? 'translate(-50%, -100%)' :
    actualPos === 'bottom' ? 'translate(-50%, 0)' :
    actualPos === 'left' ? 'translate(-100%, -50%)' :
    'translate(0, -50%)';
