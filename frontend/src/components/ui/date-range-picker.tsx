...
const PRESETS = [
  { label: 'Today', getRange: (): DateRange => {
    const d = new Date().toISOString().split('T')[0];
    return { from: d, to: d };
  }},
  { label: 'Last 7d', getRange: (): DateRange => {
    const to = new Date();
    const from = new Date();
    from.setDate(to.getDate() - 6);
    return { from: from.toISOString().split('T')[0], to: to.toISOString().split('T')[0] };
  }},
  { label: 'Last 30d', getRange: (): DateRange => {
    const to = new Date();
    const from = new Date();
    from.setDate(to.getDate() - 29);
    return { from: from.toISOString().split('T')[0], to: to.toISOString().split('T')[0] };
  }},
  { label: 'Last 90d', getRange: (): DateRange => {
    const to = new Date();
    const from = new Date();
    from.setDate(to.getDate() - 89);
    return { from: from.toISOString().split('T')[0], to: to.toISOString().split('T')[0] };
  }},
  { label: 'This Month', getRange: (): DateRange => {
    const now = new Date();
    const from = new Date(now.getFullYear(), now.getMonth(), 1);
    return { from: from.toISOString().split('T')[0], to: now.toISOString().split('T')[0] };
  }},
  { label: 'Last Month', getRange: (): DateRange => {
    const now = new Date();
      ...
      <div className="relative">
        ...
        {showPresets && (
          <>
            ...
            />
            <div className="absolute right-0 top-full mt-1 z-50 min-w-[160px] bubble border backdrop-blur-sm p-1.5"
