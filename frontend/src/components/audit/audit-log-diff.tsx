...
function computeDiff(before: Record<string, unknown>, after: Record<string, unknown>): DiffItem[] {
...
export function AuditLogDiff({ beforeState, afterState }: AuditLogDiffProps) {
  ...
  if (!hasBefore && !hasAfter) {
    return <p className="text-sm text-muted-foreground italic">No state captured</p>
  }
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className={cn('font-mono text-xs font-medium', text)}>{item.key}</span>
