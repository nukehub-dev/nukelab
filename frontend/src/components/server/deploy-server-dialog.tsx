...
export function DeployServerDialog({
  ...
  useEffect(() => {
    if (open) {
      ...
      const envId =
        defaultEnvironmentId && environments.some((e) => e.id === defaultEnvironmentId)
          ? defaultEnvironmentId
          : ''
      queueMicrotask(() => {
  ...
  const capacityPercent =
    planDiskBytes > 0 ? Math.min(100, (totalAllocatedBytes / planDiskBytes) * 100) : 0

  const isValid =
    deployForm.name.trim() && deployForm.plan_id && deployForm.environment_id && !isOverCapacity
      ...
      <DialogContent>
            ...
            >
              {environments.map((env) => (
                <SelectItem
                  key={env.id}
                  value={env.id}
                  data-testid={`deploy-server-environment-${env.id}`}
                >
                  {env.name} ({env.slug})
                </SelectItem>
              ))}
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  >
                    ...
                    {formatPlanResource(selectedPlan.disk_limit)}
                  </span>
                </div>
                ...
                {isOverCapacity && (
                  <p className="text-xs text-destructive">
                    Total volume capacity exceeds plan disk limit. Reduce sizes or choose a larger
                    plan.
                  </p>
                )}
              </div>
        ...
        <DialogFooter>
          ...
          <Button
            type="submit"
            form="deploy-form"
            data-testid="deploy-server-submit"
            loading={isPending}
            disabled={!isValid}
          >
