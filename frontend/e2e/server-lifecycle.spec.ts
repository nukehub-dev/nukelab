...
test.describe('Server lifecycle', () => {
  ...
  test('admin can deploy and stop a server', async ({ page }) => {
    ...
    const planOption = page.locator('[data-testid="select-dropdown"] button', { hasText: 'Small (2 CPU / 4g / 20 GB disk)' }).first();
    ...
    await page.waitForTimeout(500);
    const envOption = page.locator('[data-testid="select-dropdown"] button', { hasText: 'E2E Default (e2e-default)' }).first();
